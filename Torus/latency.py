...
def packet(lam, n, dim, thr=6):
    ...
    while rec < times * 200:
        ...
        for i in range(len(mark)):
            if i > (int(n/2)*dim+2)*times:
                mark[i] = -2 if mark[i - (int(n/2)*dim+2) * times] != -1 else mark[i]
...
if __name__ == "__main__":
    ...
    for j in range(num):
        for i in range(len(lam)):
            ...
            print(res[1], res[2]/res[1], res[2]/(res[1]*number*lam[i]))
        print(j)
    y = y/num
    z = z/num
    w = w/num
    ...
    file = open('cycles_bc.txt', 'w')
    for v in y:
        file.write(str(v) + '\n')
    file.close()

    file = open('Throughput_bc.txt', 'w')
    for v in z:
        file.write(str(v) + '\n')
    file.close()

    file = open('Received_bc.txt', 'w')
    for v in w:
        file.write(str(v) + '\n')
    file.close()
