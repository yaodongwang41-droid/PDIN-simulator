...
def packet(lam, n, thr=5):
    ...
    while rec < times * 200:
        ...
        for i in range(len(mark)):
            # if i > (n+2)*times:
            #     mark[i] = -2 if mark[i - (n+2) * times] == 0 else mark[i]
...
if __name__ == "__main__":
    ...
    file = open('cycles_rw.txt', 'w')
    for v in y:
        file.write(str(v) + '\n')
    file.close()

    file = open('Throughput_rw.txt', 'w')
    for v in z:
        file.write(str(v) + '\n')
    file.close()

    file = open('Received_rw.txt', 'w')
    for v in w:
        file.write(str(v) + '\n')
    file.close()
