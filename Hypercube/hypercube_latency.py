...
def routing(s, d, dct, mark, max_p=6):
    ...
    if mark == -2:  # stop sending packet in this cycle
        ...
        for i in range(len(s)):
            if s[i] != d[i]:
                a.append(i)  # save the different bits of source and destination nodes
...
if __name__ == "__main__":
    ...
    num = 40  # repeat the simulation for num times
    for j in range(num):
    ...
    file = open('cycles_rrc.txt', 'w')
    for v in y:
        file.write(str(v) + '\n')
    file.close()

    file = open('Throughput_rrc.txt', 'w')
    for v in z:
        file.write(str(v) + '\n')
    file.close()

    file = open('Received_rrc.txt', 'w')
    for v in w:
        file.write(str(v) + '\n')
    file.close()
